<?= form_open('fasilitas/hapusdetailall', ['class' => 'formhapus']) ?>


<a href="<?= base_url('fasilitas/list/') ?>" class="btn btn-info btn-sm "><i class="far fa-arrow-alt-circle-left font-14"></i> Kembali</a>
<?php if ($akses == 1) { ?>
...
<div class="table-responsive b-0">
    <table id="listdetailfasilitas" class="table table-hover table-striped">
        ...
        <tbody>
            ...
            foreach ($list as $data) :
                ...
                <tr>
                    <td class="p-1">
                        <input type="checkbox" name="fasilitasdetail_id[]" class="centang_id" value="<?= $data['fasilitasdetail_id'] ?>">
                    </td>
                    <td class="p-1"><?= $nomor ?></td>

                    <td class="p-1"><img class="img-circle elevation-2" src="<?= base_url('/public/img/informasi/fasilitas/detail/' . $data['gambar']) ?>" width="130px" height="80px"></td>

                    <td><?= esc($data['deskripsi']) ?></td>

                    <td class="text-center p-1">
                        <?php if ($akses == 1) { ?>
                            <button type="button" title="Edit Data" class="btn btn-warning btn-sm p-1" onclick="edit('<?= $data['fasilitasdetail_id'] ?>','<?= $data['fasilitas_id'] ?>')">
                                <i class="fa fa-edit"></i>
                            </button>
                            <button type="button" title="Hapus Data" class="btn btn-danger btn-sm p-1" onclick="hapus('<?= $data['fasilitasdetail_id'] ?>','<?= $data['deskripsi'] ?>')">
...
<script>
    ...
    function edit(fasilitasdetail_id, fasilitas_id) {
    ...
    function hapus(fasilitasdetail_id, transparan_nama) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "<?= site_url('fasilitas/hapusdetail') ?>",
                    type: "post",
                    dataType: "json",
                    data: {
                        [csrfToken]: csrfHash,
    ...
    $(document).ready(function() {
        ...
        $('.tambah').click(function(e) {
            ...
            $.ajax({
                url: "<?= site_url('fasilitas/formtambahdetail') ?>",
                data: {
                    fasilitas_id: fasilitas_id,
                },
                dataType: "json",
